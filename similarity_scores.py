...
save_not_crawled = "/home/gannu/Documents/n_crawled/feb_not_crawls/hardware.txt"
#counter = 0
csv_filename = "sample_file.csv"
counter = 0
with open("/home/gannu/Documents/gdrive_extract_text/more_files/WL2LHardware.txt", "r") as f:
                ...
                try:
                    ...
                    if (len(corpus)<100) or ("This site can’t be reached" in corpus) or ("document not found" in corpus) or ("No document" in corpus) or ("page not found" in corpus) or ("under construction" in corpus):
                          ...
                          # if the content from url is Not enough then  add it  to not_crawled list
                            #not_crawled.append(each_url+ str(ex))
                        ...
                        for list_name, list_values in all_the_lists:
                            ...
                            with open(csv_filename, 'a', newline='') as trail_csvFile:
                                field_names = ["url","List_Name","cos_sim","wmd_distance","wm_similarity","start_timestamp","time_taken"]
                                csv_writer = csv.DictWriter(trail_csvFile, fieldnames=field_names)
                                if not file_exists:
